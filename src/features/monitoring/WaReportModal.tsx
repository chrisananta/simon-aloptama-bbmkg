...
export const WaReportModal: React.FC<WaReportModalProps> = ({
  ...
  const formatDateIndo = (dateStr?: string) => {
    ...
    try {
      ...
      if (parts.length === 3) {
        ...
        return `${day} ${realMonths[monthIdx] || ''} ${year}`;
      }
    } catch (e) {}
    return dateStr;
  };
  ...
  return (
    <div className="fixed inset-0 z-[3000] flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4 overflow-y-auto animate-fade-in">
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-2xl overflow-hidden my-6 flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-heading text-lg font-bold">Format Laporan WA Grup</h3>
        ...
        <div className="p-5 overflow-y-auto flex-1 space-y-3">
          ...
          <textarea
            ...
            className="w-full h-80 p-4 bg-slate-900 text-emerald-300 font-mono text-xs rounded-xl border border-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 resize-none leading-relaxed shadow-inner"
          />
        </div>
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-200 flex flex-wrap items-center justify-between gap-3 shrink-0">
          <button
            ...
            className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white font-bold text-xs px-4 py-2.5 rounded-xl shadow-xs transition-colors cursor-pointer"
          >
            <Share2 size={15} />
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handleCopy}
              className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-xs text-white transition-all shadow-md cursor-pointer ${
                copied
                  ? 'bg-emerald-600'
