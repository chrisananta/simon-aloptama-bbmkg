  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      ...
      <style>
        ...
        .page { background: white; padding: 25px; border-radius: 8px; max-width: 950px; margin: auto; }
    ...
    <div className="fixed inset-0 z-[3000] flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-2 sm:p-4 animate-fade-in overflow-y-auto">
      <div className="bg-white rounded-2xl w-full max-w-5xl flex flex-col max-h-[92vh] overflow-hidden shadow-2xl my-auto">
        ...
        <div className="bg-[#0A203C] text-white p-4 flex justify-between items-center shrink-0">
          <h3 className="font-bold text-sm sm:text-base">Laporan Rekapitulasi Pemeliharaan Genset</h3>
          <button onClick={onClose} className="p-1.5 hover:bg-white/20 rounded-lg text-slate-300 hover:text-white cursor-pointer"><X size={18}/></button>
        </div>
        
        {/* Toolbar Filter */}
        <div className="p-3.5 bg-slate-50 border-b flex gap-3 items-center flex-wrap text-xs font-bold text-slate-700 shrink-0">
          <Filter size={15} className="text-[#0052CC]" />
          <div className="flex items-center gap-1.5">
            <span>Dari:</span>
            <input type="date" value={startDate} onChange={e=>setStartDate(e.target.value)} className="p-1.5 bg-white border border-slate-300 rounded-lg outline-none focus:border-[#0052CC]" />
          </div>
          <div className="flex items-center gap-1.5">
            <span>Sampai:</span>
            <input type="date" value={endDate} onChange={e=>setEndDate(e.target.value)} className="p-1.5 bg-white border border-slate-300 rounded-lg outline-none focus:border-[#0052CC]" />
          </div>
          <div className="flex items-center gap-1.5">
            <span>Filter Petugas:</span>
            <select value={selectedPetugas} onChange={e=>setSelectedPetugas(e.target.value)} className="p-1.5 bg-white border border-slate-300 rounded-lg outline-none focus:border-[#0052CC] cursor-pointer">
              <option value="ALL">Semua Petugas ({petugasOptions.length})</option>
              {petugasOptions.map(p => <option key={p} value={p}>{p}</option>)}
            </select>
          </div>
          <button onClick={handlePrint} className="ml-auto px-4 py-2 bg-[#0052CC] hover:bg-blue-800 text-white rounded-xl shadow-xs flex items-center gap-1.5 cursor-pointer">
        ...
        <div className="p-6 overflow-y-auto flex-1 bg-slate-100">
          <div id="genset-report-printable" className="bg-white p-8 border border-slate-300 shadow-sm mx-auto text-black">
            ...
            <h3 className="text-center font-black text-sm uppercase mb-4 tracking-wide">
              LAPORAN REKAPITULASI PEMELIHARAAN &amp; MONITORING GENSET
            </h3>
            
            {/* Kotak Informasi dengan Tanda : Rapi & Sejajar */}
            <div className="mb-4 text-xs font-bold bg-slate-50 p-3 border border-black/20 rounded-md">
